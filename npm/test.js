...
describe("Platform Suffix", function () {
  ...
  it("should throw for unsupported platform", function () {
    assert.throws(() => {
      lib.getSuffix("BadType", "BadArch");
    });
  });
});

describe("Binary Name", function () {
  it("should return forge-cli.exe for Windows_NT", function () {
    let result = lib.getBinaryName("Windows_NT", "");
    assert.equal(
      result,
      "forge-cli.exe",
      `Expected 'forge-cli.exe', but got: ${result}`
    );
  });
  it("should return forge-cli for Linux", function () {
    let result = lib.getBinaryName("Linux", "x64");
    assert.equal(
      result,
      "forge-cli",
      `Expected 'forge-cli', but got: ${result}`
    );
  });
  it("should return forge-cli for Darwin", function () {
