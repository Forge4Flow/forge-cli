...
func init() {
	...
	templatePull, _, _ := forgeCmd.Find([]string{"template", "pull"})
...
var templateStorePullCmd = &cobra.Command{
	...
	Example: `  forge-cli template store pull ruby-http
  forge-cli template store pull go --debug
  forge-cli template store pull openfaas/go --overwrite
  forge-cli template store pull golang-middleware --url https://raw.githubusercontent.com/openfaas/store/master/templates.json`,
